...
#############################
	...
	exec { 'rm default site' :
 	...
 	file { 'Content default web site' :
		...
		source  => 	'puppet:///files/config/nginx/index',
		ensure 	=> 	directory,
		recurse 	=>	'true',
		owner		=> NGINX,
...
#####################################
	...
	file { '/etc/nginx/nginx.conf' :
		source  => 'puppet:///files/config/nginx/nginx.conf',
		replace =>      true,
	...
	file { '/etc/nginx/geoip.dat' :
		source  => 'puppet:///files/config/nginx/geoip.dat',
		replace =>      true,
		notify  =>      Service["nginx"],
		owner => NGINX,
    }	
	...
	file { '/etc/nginx/sites-available/template' :
		source  => 'puppet:///files/config/nginx/template',
		replace =>      true,
		notify  =>      Service["nginx"],
		owner 	=> NGINX,
    }
